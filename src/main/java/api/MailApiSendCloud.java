...
public class MailApiSendCloud {
    ...
    // 外部调用发送方法
    // 只提供发送邮箱
    public boolean sendMail(String httpUrl, Student student) {

        return sendMailReal(student, subject, httpUrl, fromName);
    }

    public boolean sendMail(String httpUrl, Student student, String toSubject) {
        return sendMailReal(student, toSubject, httpUrl, fromName);
    }

    public boolean sendMail(String httpUrl, Student student, String toSubject, String fromName) {
        return sendMailReal(student, toSubject, httpUrl, fromName);
    }

    // 发送方法
    private boolean sendMailReal(Student student, String subject, String httpUrl, String fromName) {
        // 拼接验证url httpUrl 当前项目的根目录
        randInt = RandomTool.getRandomCode(10);
        String sendBody = sendBodyBegin + httpUrl + "/"+randInt + "/"+student.getId()+ sendBodyEnd;
        logger.debug("拼接发送内容: " + sendBody);
        // 设置发信内容
        params = new ArrayList<NameValuePair>();
        params.add(new BasicNameValuePair("apiUser", apiUser));
        params.add(new BasicNameValuePair("apiKey", apiKey));
        params.add(new BasicNameValuePair("to", student.getMail()));
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if ((Integer)jsonObject.get("statusCode") == 200){
                   ...
                   logger.info("邮箱缓存存入时: " + student.toString());
                    Memcached.add(randInt, student);
